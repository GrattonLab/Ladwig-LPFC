...
% Remove axis labels (equivalent to hiding them in heatmap)
set(gca, 'XTick', [], 'YTick', []);
caxis([0.2 0.6]);  % Set color limits
...
zeros(0,1), ...% within
zeros(0,1), ...% within std
zeros(0,1), ...% between 
zeros(0,1), ...% between std
zeros(0,1), ...   % PValue
zeros(0,1), ...   % CorrectedPValue
zeros(0,1), ...   % TStat
zeros(0,1), ...   % CohensD
zeros(0,1), ... % df 
'VariableNames', {'Comparison','Within mean','Within SEM', 'Between mean', 'Between SEM', 'PValue', 'CorrectedPValue', 'TStat', 'CohensD', 'df'});

stats_row = {'within vs. between',average_within_subject, stderr_within_subject, average_between_subject_within_task, stderr_between_subject_within_task, p,p,stats.tstat,stats.tstat/sqrt(stats.df+1), stats.df};
